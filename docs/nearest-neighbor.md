...
**Complexity:** O(N log N) for typical geographic distributions (uniformly distributed points on a sphere); O(N²) worst case (degenerate insertion orders). Note: the implementation uses deterministic insertion order, not randomized — the O(N log N) bound relies on input distribution rather than algorithmic randomization. FaceGrid provides O(1) amortized face lookup. The implementation uses multiple fallback strategies (greedy walk → FaceGrid lookup → BFS → linear scan) to handle the pathological cases gracefully.
