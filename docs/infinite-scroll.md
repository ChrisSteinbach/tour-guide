...
- [UX Guarantees](#ux-guarantees) — what a maintainer can promise the user
  - [How It Starts](#how-it-starts)
  - [Scroll Mode Transitions](#scroll-mode-transitions)
  - [Scroll Headroom: Avoiding the Bottom Boundary](#scroll-headroom-avoiding-the-bottom-boundary)
- [Implementation](#implementation) — how those guarantees are kept
  - [Virtual Scroll](#virtual-scroll)
  - [Data Model: ArticleWindow](#data-model-articlewindow)
  - [Near-End Detection and Expansion](#near-end-detection-and-expansion)
  - [Enrichment and Map Sync](#enrichment-and-map-sync)
...
3. **`loadedCount` in `onWindowChange`** — When tiles finish loading, the callback writes `lastScrollCount = articleWindow.loadedCount()` directly (no ratchet at the article-window-lifecycle layer), allowing truth-settles to correct the optimistic count upward. A downward correction is still possible in principle: `evictIfNeeded` in `article-window.ts` can drop tail entries on a back-to-top fetch and shrink `loadedCount()` by a handful of items. The infinite-scroll lifecycle's `update()` therefore applies a non-decreasing clamp (`currentLoadedCount = max(currentLoadedCount, loadedCount)`), so the near-end gate stays anchored to the high-water mark and the list height never shrinks. The high-water mark resets on `resetArticleWindow` (new position or query).
...
**Regression test net:** `createVirtualList` accepts an optional `maxSafeScrollHeight` override that lets the suite force compressed mode at a small totalCount. A sweep test in `virtual-scroll.test.ts` walks scrollTop across the full scrollable range, asserts indices and non-overlapping tops at each sample, and pins the fine-scroll reposition invariant (`5add52a`) so a future "only re-render on range change" regression is caught up front.
