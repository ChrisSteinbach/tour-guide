...
| ------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Tile fetch failure**                                                                           | `loadTile` retries transient failures (network errors, 5xx/429) with backoff before giving up; the effect executor then dispatches `tileLoadFailed`, tagged `primary` when it was the nearest tile for the position. The state machine sets `primaryTileFailed` on a primary failure, so the renderer shows a dismissible degraded notice (nearest tile failed but adjacent tiles still filled the list from farther out) or an explicit empty-state retry (nothing loaded); the radar labels its empty caption accordingly. Retry re-drives `loadTiles`, which is the only retry path in picked-position mode (no GPS ticks). | Nearest results may be missing or distant, but a notice explains it and one tap retries; a fully failed load shows an empty list with a retry button rather than a silent blank |
| **Binary deserialization failure** (corrupt `.bin`, truncated download, format version mismatch) | `deserializeBinary` validates magic bytes (`"SDLT"`), format version, and section bounds — throws `BinaryFormatError` on any mismatch; `decodeArticlePayload` separately validates the article payload, throwing a plain `Error` on corruption; `loadTile` records the failure in the session tile-log (`recordTileLoad`, which also emits a `console.warn`) before re-throwing, so a fetch that succeeded but produced unusable bytes is not silently marked ok; the effect executor turns the rejection into `tileLoadFailed`; app continues with remaining tiles                                                            | Same as tile fetch failure — results from other tiles still display; corrupt tile is not cached in IDB                                                                          |
...
| **Wikipedia API 429** (rate limit)                                                               | `wiki-api.ts` throws `RateLimitError` carrying the parsed `Retry-After` header (or `DEFAULT_RETRY_AFTER_MS = 30 min` if missing). `summary-loader.ts` sets a module-global `rateLimitUntil` that pauses the entire enrichment queue until the window reopens; the rate-limited title is re-queued so it retries automatically. `cancel()` clears the local queue but **deliberately preserves** `rateLimitUntil` because it reflects remote-server state                                                                                                                                                                       | Article cards show title + distance only until the window expires (~30 min); detail view still works for cached items; no error is surfaced to the user                         |
...
- **Tile fetches retry, then degrade visibly:** `loadTile` retries transient failures (network errors, HTTP 5xx/429) with backoff; a genuinely exhausted fetch dispatches `tileLoadFailed`. The app shows results from whichever tiles succeed. A failed _primary_ (nearest) tile sets `primaryTileFailed`, which surfaces a dismissible degraded notice or an empty-state retry rather than a silent blank, and lets the user re-drive `loadTiles` on demand — the only retry path in picked-position mode, where there are no GPS ticks to re-request tiles.
- **429 circuit breaker is global and process-lifetime:** Wikipedia's rate limit is per-client across the whole REST endpoint, so `summary-loader.ts` keeps `rateLimitUntil` as module-global state and pauses the entire enrichment queue when any in-flight request 429s. `rateLimitUntil` deliberately survives `cancel()` because it reflects remote-server state — a fresh `load()` after a position change still has to honor the open window. The next `drain()` schedules a resume timer with `setTimeout` so the queue automatically restarts when `Retry-After` elapses.
...
- [The Browse List](infinite-scroll.md) — Virtual scroll, the browse list's levels of detail, scroll-pause detection
