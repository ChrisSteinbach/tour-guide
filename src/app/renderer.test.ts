...
describe("renderer renderInfiniteScrollDOM loadedCount fallback", () => {
  it("uses ArticleWindow loadedCount when greater than zero", () => {
    // When the ArticleWindow has loaded data, totalCount === loadedCount.
    // The state-machine articles are NOT used as a placeholder.
    const aw = stubArticleWindow({ loadedCount: vi.fn(() => 73) });
    const init = vi.fn();
    const infiniteScroll = stubInfiniteScroll({
      isActive: vi.fn(() => false),
      init,
    });
    // Give the state machine more articles than the window so we can prove
    // the fallback is NOT being used.
    const deps = makeDeps({
      getState: vi.fn(() =>
        tiledBrowsingState(
          {},
    ...
    expect(init).toHaveBeenCalledWith(73);
  });

  it("falls back to state.phase.articles.length when loadedCount is 0", () => {
    // Before the first fetch completes, the ArticleWindow loadedCount is 0.
    // The renderer falls back to the state-machine articles count so the
    // virtual list has SOME headroom rather than collapsing to height 0.
    // See docs/infinite-scroll.md "Scroll Headroom" for the rationale.
    const aw = stubArticleWindow({ loadedCount: vi.fn(() => 0) });
    const init = vi.fn();
    const infiniteScroll = stubInfiniteScroll({
      isActive: vi.fn(() => false),
      init,
    ...
    expect(init).toHaveBeenCalledWith(12);
  });

  it("falls back to state.phase.articles.length when no ArticleWindow exists", () => {
    // No ArticleWindow at all — the renderer must use the state machine
    // articles for the placeholder count, never crash on aw.loadedCount().
    const init = vi.fn();
    const infiniteScroll = stubInfiniteScroll({
      isActive: vi.fn(() => false),
      init,
    ...
    expect(init).toHaveBeenCalledWith(2);
  });
});
