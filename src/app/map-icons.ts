...
// Shared teardrop shell for every pin marker — a single source of truth so the
// Wikipedia article pins, the yellow highlight pin, and the picker location pin
// all have identical silhouettes and only differ in fill/contents.
...
function locationPin(fill: string, w: number, h: number): L.Icon {
  const contents = `<path d="${PIN_SHELL_PATH}" fill="${fill}" filter="url(#s)"/><circle cx="256" cy="185" r="56" fill="#fff"/>`;
  return makePinIcon(contents, w, h);
}

export const WIKI_PIN_SIZE: [number, number] = [30, 42];
export const WIKI_PIN_HIGHLIGHT_SIZE: [number, number] = [36, 50];
export const LOCATION_PIN_SIZE: [number, number] = [30, 42];

export const wikiPinIcon = wikiPin("#fff", ...WIKI_PIN_SIZE);
export const wikiPinHighlightIcon = wikiPin(
  "#FFC107",
