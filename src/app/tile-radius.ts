...
const MAX_ROW = Math.floor(180 / 5) - 1; // 35
const MAX_COL = Math.floor(360 / 5) - 1; // 71
...
export function createTileRadiusProvider(
  ...
  return {
    async fetchRange(start: number, end: number): Promise<FetchResult> {
      ...
      while (!noMoreTiles) {
        const articles = await queryAllTiles();
        if (articles.length >= end) {
          // Sort by distance and return the requested slice
      ...
      // No more tiles — return what we have
      const articles = await queryAllTiles();
      articles.sort((a, b) => a.distanceM - b.distanceM);
      return {
